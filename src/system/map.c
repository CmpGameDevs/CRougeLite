...
{
  GameState *game_system = gameState;
  Enemy *enemies = game_system->enemies;
  Map *map = &(game_system->map);
  ...
  {
    drawPath(enemies[i].ai.path, enemies[i].ai.pathLength, colors[i % 6]);
    ...
    bool playerDetected = enemies[i].ai.state == RUN;
    Color rangeOutline = playerDetected ? RED : SKYBLUE;
    Color rangeFill = playerDetected ? 
                     ColorAlpha(RED, 0.2f) : 
                     ColorAlpha(SKYBLUE, 0.1f);
    
    // Draw detection range with both fill and outline
    DrawCircle(enemyCenterPos.x, enemyCenterPos.y, 
              enemies[i].ai.detectionRange, rangeFill);
    DrawCircleLines(enemyCenterPos.x, enemyCenterPos.y, 
                   enemies[i].ai.detectionRange, rangeOutline);
...
bool isTileWalkable(int tileId)
{
  ...
  if (props & TILE_PROP_WALKABLE)
    return true;
  if (props & TILE_PROP_SOLID)
    return false;
...
{
  if (!mapping || mapping->lootCount == 0)
    return;
  GameState *game_system = gameState;
  Map *map = &(game_system->map);
  int dx[] = {0, 1, 0, -1};
